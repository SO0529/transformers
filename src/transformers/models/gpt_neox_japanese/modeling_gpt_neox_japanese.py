...
class GPTNeoXJapaneseLayer(nn.Module):
    def __init__(self, config, layer_number):
        ...
        self.attention = GPTNeoXJapaneseAttention(
            config=config,
            bias=layer_number == config.num_hidden_layers - 1
        )
...
@add_start_docstrings(
    """GPTNeoXJapanese Model with a `language modeling` head on top for Classifier Model fine-tuning.""", GPT_NEOX_JAPANESE_START_DOCSTRING
)
